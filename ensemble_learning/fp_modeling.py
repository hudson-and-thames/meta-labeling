...
# create empty csv file using with open
# with open('hyp2.csv', 'w') as new_csv_file:
#    pass

df = pd.read_csv('./hyp2.csv', index_col=0)
df_empty = df[0:0]
df_empty.to_csv('hyp2.csv')
...
#    test['prob_regime'] = meta_model_regime.predict_proba(X_test_regime_scaled)[:, 1]
    ...
    # --- Statistics ---
    # -----------------------------------------------------------------------------------------------
    # Primary model stats: precision, recall, f1_score, support, accuracy, auc, macro_avg_f1, weighted_avg_f1, specificity
    brow = classification_stats(actual=test['target'], predicted=test['pmodel'], prefix='b',
                                get_specificity=False)
    # Information Advantage: precision, recall, f1_score, support, accuracy, auc, macro_avg_f1, weighted_avg_f1, specificity
    irow = classification_stats(actual=y_test, predicted=test['pred_info'], prefix='mi',
                                get_specificity=True)
    # False Positive Modeling: precision, recall, f1_score, support, accuracy, auc, macro_avg_f1, weighted_avg_f1, specificity
    fprow = classification_stats(actual=y_test, predicted=test['pred_regime'], prefix='fp',
                                 get_specificity=True)
    # Concat data
    final_row = pd.concat([brow, irow, fprow], axis=1)

    # Add Strategy Metrics: mean, stdev, sr for bah, primary, meta_info, meta_regime
    ...
    # Comparison Metrics: difference between imeta | fmeta - primary for sr, mean, stdev
    ...
    # Classification Metrics: : difference between imeta | fmeta - primary for recall, precision, accuracy, f1, auc
    ...
    # Save results to csv
    data = pd.read_csv('hyp2.csv', index_col=0)
    concat = pd.concat([data, final_row]).reset_index(drop=True)
    concat.to_csv('hyp2.csv')
