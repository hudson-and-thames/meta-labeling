...
from sklearn.metrics import classification_report, roc_curve, confusion_matrix, RocCurveDisplay, ConfusionMatrixDisplay, roc_auc_score, PrecisionRecallDisplay, precision_recall_curve, accuracy_score
from sklearn.preprocessing import StandardScaler
from sklearn.ensemble import RandomForestClassifier
from deslib.des import DESP, KNORAU, KNORAE, KNOP
from meta_labeling.data_generation import single_regime, vol_regime, prep_data, classification_stats, add_strat_metrics

df = pd.read_csv('./pool1.csv', index_col=0)
df_empty = df[0:0]
df_empty.to_csv('pool1.csv')
...
for z in range(0, 1000, 1):
    ...
    # Train model
    X_train, X_dsel, y_train_en, y_dsel = train_test_split(X_train_scaled, y_train, test_size=0.40, shuffle=False)
    ...
    pool_classifiers = [model_nb, model_lr, model_dt, model_svc, model_mlp]
    ...
    # pool = [LogisticRegression(), DecisionTreeClassifier(), GaussianNB()]
    # for c in pool:
    #     c.fit(X_train, y_train)
    # meta_model = KNORAU(pool_classifiers=pool)
    meta_model = knorau
    ...
    data = pd.read_csv('./pool1.csv', index_col=0)
    concat = pd.concat([data, final_row]).reset_index(drop=True)
    concat.to_csv('pool1.csv')
