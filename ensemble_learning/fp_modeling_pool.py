...
from deslib.des import DESP, KNORAU, KNORAE, KNOP

from meta_labeling.data_generation import vol_regime, prep_data, classification_stats, add_strat_metrics

df = pd.read_csv('./pool2.csv', index_col=0)
df_empty = df[0:0]
df_empty.to_csv('pool2.csv')
...
for z in range(0, 1000):
    ...
    # Min_max_scaler_i = MinMaxScaler().fit(X_train_info)
    # Min_max_scaler_r = MinMaxScaler().fit(X_train_regime)
    # ## Scaling
    # X_train_mm_scaled_i = Min_max_scaler_i.transform(X_train_info)
    # X_train_mm_scaled_r = Min_max_scaler_r.transform(X_train_regime)
    #
    # X_test_mm_scaled_i = Min_max_scaler_i.transform(X_test_info)
    # X_test_mm_scaled_r = Min_max_scaler_r.transform(X_test_regime)

    # Train 2 models (Info, FP)
    X_train_i, X_dsel_i, y_train_en_i, y_dsel_i = train_test_split(X_train_info
                                                                   , y_train, test_size=0.40, shuffle=False)
    X_train_r, X_dsel_r, y_train_en_r, y_dsel_r = train_test_split(X_train_regime, y_train, test_size=0.40, shuffle=False)
    ...
    # model_mlp.fit(X_train, y_train_en)

    pool_classifiers_info = [model_nb, model_lr, model_dt, model_svc, model_mlp]
    pool_classifiers_regime = [model_nb, model_lr, model_dt, model_svc, model_mlp]
    
    desp_info = DESP(pool_classifiers_info)
    desp_regime = DESP(pool_classifiers_regime)
    knorau_info = KNORAU(pool_classifiers_info)
    knorau_regime = KNORAU(pool_classifiers_regime)
    kne_info = KNORAE(pool_classifiers_info)
    kne_regime = KNORAE(pool_classifiers_regime)

    des_models_info = [desp_info, knorau_info, kne_info]
    des_models_regime = [desp_regime, knorau_regime, kne_regime]

    pool_info = [LogisticRegression(), DecisionTreeClassifier(), GaussianNB(), SVC(probability=True), MLPClassifier()]
    for c in pool_info:
        c.fit(X_train_i, y_train_en_i)
    meta_model_info = KNORAU(pool_classifiers=pool_info)
    # meta_model_info = KNORAU(pool_classifiers_info)

    pool_regime = [LogisticRegression(), DecisionTreeClassifier(), GaussianNB(), SVC(probability=True), MLPClassifier()]
    for c in pool_regime:
        c.fit(X_train_r, y_train_en_r)
    meta_model_regime = KNORAU(pool_classifiers=pool_regime)
    # meta_model_regime = KNORAU(pool_classifiers_regime)
...
#    test['prob_regime'] = meta_model_regime.predict_proba(X_test_regime_scaled)[:, 1]
    ...
    # Save results to csv
    data = pd.read_csv('./pool2.csv', index_col=0)
    concat = pd.concat([data, final_row]).reset_index(drop=True)
    concat.to_csv('pool2.csv')
