...
df = pd.read_csv('./des1.csv', index_col=0)
df_empty = df[0:0]
df_empty.to_csv('des1.csv')
...
for z in range(0, 1000, 1):
    ...
    # Train model
    X_train_en, X_dsel, y_train_en, y_dsel = train_test_split(X_train_scaled, y_train, test_size=0.40, shuffle=False)

    #meta_model = LogisticRegression(random_state=0, penalty='none')
    pool = RandomForestClassifier(n_estimators=100)
    pool.fit(X_train_en, y_train_en)
    meta_model = KNORAU(pool)
    ...
    data = pd.read_csv('./des1.csv', index_col=0)
    concat = pd.concat([data, final_row]).reset_index(drop=True)
    concat.to_csv('des1.csv')
