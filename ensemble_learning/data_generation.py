...
sd_low = 0.014543365294448746  # About the same as IBM stdev
sd_high = sd_low * 2


def _gen_data(phi1, phi2, phi3, flag, stdev, drift, steps):
    """
    Helper function to generate a time series of returns data using a single AR(3) process.
...
def _gen_regime(steps, inner_steps, prob_switch, stdev):
    """
    Helper function to generate a time series of returns data using a dual AR(3) process.
    ...
    for _ in range(0, steps):
        ...
        rand_sign = np.random.uniform()
        is_regime_two = rand_sign < prob_switch
        rand_vol = np.random.uniform()
        is_regime_high_vol = rand_vol < prob_switch
        ...
        elif is_regime_two and is_regime_high_vol:
            # This negative regime has negative sign coefficients to the original and high vol
            rets_regime, flags_regime = _gen_data(phi1=PN1, phi2=PN2, phi3=PN3,
                                                  flag=1, steps=inner_steps,
                                                  stdev=sd_high, drift=-0.0001)

        elif not is_regime_two and is_regime_high_vol:
            # This negative regime has positive sign coefficients like the original and high vol
            rets_regime, flags_regime = _gen_data(phi1=PN1, phi2=PN2, phi3=PN3,
                                                  flag=1, steps=inner_steps,
                                                  stdev=sd_high, drift=-0.0001)
...
def vol_regime(total_steps, prob_switch, stdev):
    """
    Generate a time series sampled from a dual regime.
    ...
    # Gen dual regime data
    rets, flags = _gen_regime(steps=steps, inner_steps=inner_steps,
                              prob_switch=prob_switch, stdev=stdev)
