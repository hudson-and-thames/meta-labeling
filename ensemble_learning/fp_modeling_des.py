...
df = pd.read_csv('./des2.csv', index_col=0)
df_empty = df[0:0]
df_empty.to_csv('des2.csv')
...
#    X_test_regime_scaled = scaler.transform(X_test_regime)
    ...
    X_train_i, X_dsel_i, y_train_en_i, y_dsel_i = train_test_split(X_train_info
                                                                   , y_train, test_size=0.40,
                                                                   shuffle=False)
    X_train_r, X_dsel_r, y_train_en_r, y_dsel_r = train_test_split(X_train_regime, y_train, test_size=0.40,
                                                                   shuffle=False)

    pool_info = RandomForestClassifier(n_estimators=100)
    pool_info.fit(X_train_i, y_train_en_i)
    meta_model_info = KNORAU(pool_classifiers=pool_info)

    pool_regime = RandomForestClassifier(n_estimators=100)
    pool_regime.fit(X_train_r, y_train_en_i)
...
#    test['prob_regime'] = meta_model_regime.predict_proba(X_test_regime_scaled)[:, 1]
    ...
    # Save results to csv
    data = pd.read_csv('./des2.csv', index_col=0)
    concat = pd.concat([data, final_row]).reset_index(drop=True)
    concat.to_csv('des2.csv')
